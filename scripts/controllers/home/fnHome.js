...
export function GridWorkExperience() {
  ...
  if (container) {
    ...
    translations.workExperience.forEach((work) => {
      ...
      article.addEventListener("click", () => CardWorkCover(work.id));
  ...
  if (selectedEducation) {
    ...
    content.innerHTML = `
      <header class="modal-header">
        ...
        <h4>${selectedEducation.institution}</h4>
      </header>

      <hr class="modal-divider">

      <div class="modal-details">
        <p><strong>Tipo:</strong> ${selectedEducation.type}</p>

        <p><strong>Fecha:</strong> ${selectedEducation.date}</p>

        <p><strong>Descripción:</strong> ${selectedEducation.description}</p>

        <div>
          <strong>Habilidades Adquiridas:</strong>
          <ul class="tech-list">${skillsList}</ul>
        </div>

        ${selectedEducation.certificate ? `
          <p>
            <strong>Certificado:</strong> 
            <a href="${selectedEducation.certificate}" target="_blank" rel="noopener noreferrer" class="certificate-link">
